package com.pmu2.exec.infrastructure.kafka.consumer;
 ...
 */
@Component
class PmuCourseConsumer {

    private static final Logger log = LoggerFactory.getLogger(PmuCourseConsumer.class);
    ...
    public void listenKafkaMessages(@Payload CourseRecord courseRecord,
        ...
        try {
            ...
            throw new BusinessException("Exception processing course record: " + courseRecord + " error: " + e.getMessage());
        }
    }
    ...
    public void handleDltCourseRecord(@Payload CourseRecord message, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic) {
        if (log.isInfoEnabled()) {
            ...
            log.info("DTL TOPIC message : {}, topic name : {} , at: {}", message, topic, timestamp);
        }
